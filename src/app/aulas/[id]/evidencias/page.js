...
export default function AulaEvidenciasPage({ params }) {
  const { id } = use(params);
    ...
    <div>
      <div className="page-header">
        ...
        <div className="page-header-top">
          <div>
            <h1 className="page-title">📂 Evidencias — {classroom?.name}</h1>
            <p className="page-subtitle">Portafolios de {classroom?.age} · {students.length} estudiantes</p>
          </div>
      ...
      {students.length > 0 ? (
        <div className="grid-3">
          {students.map(s => (
            <Link key={s.id} href={`/aulas/${id}/evidencias/${s.id}`} className="card card-clickable">
