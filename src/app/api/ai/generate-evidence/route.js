...
import { generateEvidenceDescription } from '@/lib/openai';
...
export async function POST(request) {
  try {
    ...
    const { estudiante, actividad, competencia, nivel, observacion } = body;

    if (!observacion) {
      return NextResponse.json({ error: 'La observación es requerida' }, { status: 400 });
    }

    const result = await generateEvidenceDescription({
